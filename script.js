...
//用assign 后面覆盖前面并且合并放到前面的变量（target 被）
...

{
  ...
  console.log(arrayName.join(''));//用''替换逗号，结果和原来name一样：Kristy Chen
  console.log(arrayName.join('_'));//K_r_i_s_t_y__ C_h_e_n 中间包含空格
  console.log(newArrayName.join(''));//KristyChen 没有空格
  console.log(newArrayName.join('_'));// K_r_i_s_t_y_C_h_e_n 
...
//拼接
{
  ...
  const array3 = array1.concat(array2) //拼合在一块不会覆盖，但要一个个写
  ...
  console.log(Array.from(string1));//'a', 'b', 'c','c', 'd', 'c', 'f'//可以无限连加，一次性的。先join再拆开
}
...

{
  ...
  console.log(Object.keys(obj));//Object自带属性123
...
//sort 大小排序
{
  ...
  const newList2 = list.sort((a, b) => b - a)//从大道小排
  console.log(list);
  console.log(newList2);
